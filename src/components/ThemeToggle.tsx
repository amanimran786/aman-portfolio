...
export default function ThemeToggle() {
  ...
  return (
    ...
    >
      {/* Sun icon */}
      <span className={`absolute left-1.5 text-xs transition-opacity duration-300 ${isDark ? 'opacity-40' : 'opacity-100'}`}>
        ☀️
      </span>
      {/* Moon icon */}
      <span className={`absolute right-1.5 text-xs transition-opacity duration-300 ${isDark ? 'opacity-100' : 'opacity-40'}`}>
        🌙
      </span>
      {/* Sliding knob */}
      <motion.div
